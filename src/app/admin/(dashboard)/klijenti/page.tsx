    ...
    <div>
      <div className="mb-8 flex items-center justify-between gap-4">
        ...
        <form className="flex gap-2">
          ...
          <button
            type="submit"
            className="border border-ink/20 px-4 py-2.5 font-mono text-xs tracking-[0.08em] text-ink-dim uppercase hover:text-ink"
          >
            Pretraži
          </button>
        </form>
        ...
        <table className="w-full text-left font-mono text-sm">
          ...
          <tbody>
            {customers?.map((customer) => (
              <tr key={customer.id} className="hover:bg-ink/[0.03]">
                <td className="border-b border-ink/10 px-4 py-3">
                  <Link
                    href={`/admin/klijenti/${customer.id}`}
                    className="text-ink underline decoration-ink/30 underline-offset-2 hover:text-brass"
                  >
                    {customer.name}
                  </Link>
                </td>
